...
function extractErrorCode(message: string | null | undefined): string | null {
  ...
  // Must match SQL exceptions thrown by mark_in_person_scan_completed()
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  /**
   * IMPORTANT:
   * This endpoint is the irreversible commitment point:
   * - user has entered data
   * - now we unlock / start analysis
   * - we spend 1 credit here
   *
   * We use Service Role so the RPC can always write ledger rows.
   * The RPC itself still enforces auth.uid() so credits can only be spent
  ...
  if (error) {
    ...
    console.error("Failed to unlock / start analysis:", error);
